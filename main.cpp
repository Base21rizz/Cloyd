...
{
    screenPos = {
        (float)winWidth / 2.0f - 4.0f * (0.5 * (float)texture.width / 6.0),
        (float)winHeight / 2.0f - 4.0f * (0.5 * (float)texture.height)};
}
...
void Character::tick(float deltaTime)
{
    ...
    Rectangle source{frame * (float)texture.width / 6.f, 0.f, rightLeft * (float)texture.width / 6.f, (float)texture.width / 6.f};
